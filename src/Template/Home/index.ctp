<?php
  ...
  $this->assign('title', __('青色申告決算書 &amp; 仕訳帳システム'));
  
?>
<p></p>
<h2>1. このシステムの流れ</h2>
<p></p>
<p>初めての方は<a href="<?= $this->Url->build('/capitals', true) ?>">会計年度</a>で会計年度を新規登録します。<br>次に<a href="<?= $this->Url->build('/capitals', true) ?>">会計年度</a>の画面から該当年の「仕訳帳」を選択して「仕訳」を新規登録していきます。</p>
...
<p style="margin-top:24px;margin-bottom:24px;">
  <?php foreach ($capitals as $index =>$capital){ ?>
    <span><a href="<?= $this->Url->build('/?yyyy=' .$capital->yyyy . '#report', true) ?>"><?= $capital->yyyy ?>年度</a></span>&nbsp;&nbsp;
    <?php if(($index+1) % 3 === 0) { ?>
...
<?php if(isset($yyyy)) { ?>
  ...
  <table class="table table-hover">
    ...
    <tbody>                  
      <?php foreach($report_month as $index => $item) { ?>
      <tr>
         <td><?= ($index+1) ?>月</td>
         <td><?= number_format($item['money']) ?></td>
      </tr>             
